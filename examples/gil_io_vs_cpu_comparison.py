...
Threads are excellent for I/O-bound jobs because one thread can run while
another waits on the network, disk, or a database. They do not provide the same
benefit for CPU-bound Python code because of the Global Interpreter Lock (GIL).
"""
...
def run_with_queue(task_obj, count):
    task_ids = []
    start = time.time()
    for item in range(count):
...
def main():
    ...
    try:
        ...
        io_queue = run_with_queue(call_shipping_rate_api, count)

        cpu_sequential = run_sequential(score_fraud_risk, count)
        cpu_queue = run_with_queue(score_fraud_risk, count)
